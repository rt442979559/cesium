...
export function drawFlyLine() {
  const viewer = getInstance()
  const Cesium = getCesium()

  // 这里通过算法得到曲线

  const mm = parabola([108.89574878031878, 20.23612176742662, 120.32036514618619, 29.026845281833523])
...
function parabola(twoPoints) { // 抛物线绘制
  ...
  const a:any = (heigh / (x2 * x2))
  function y(x:any, height:any) { return height - a * x * x }
...
function getFlylineMaterial() {
  ...
  material.uniforms.color = new Cesium.Color.fromCssColorString(
    'rgba(0, 0, 0, 1)'
  )
  // 飞线效果-飞线间隔，宽度2
